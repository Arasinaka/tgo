    ...
    for (const binding of Object.values(events)) {
      if (Array.isArray(binding)) {
        for (const item of binding) {
          if (item && typeof item.action === 'string') names.add(item.action)
        }
      } else if (binding && typeof binding.action === 'string') {
...
export default function JSONRenderSurface({ spec, onAction }: JSONRenderSurfaceProps) {
